package cn.bidlink.framework.util.holder;
...
import cn.bidlink.framework.core.config.EncryptedPropertiesFactoryBean;
import cn.bidlink.framework.core.exceptions.GeneralException;
import cn.bidlink.framework.core.utils.CollectionUtils;
...
public class PropertiesHolder implements ApplicationContextAware, ServletContextAware, InitializingBean {
	...
	public void afterPropertiesSet() throws Exception {
		...
		if (servletContext != null) {
			...
			servletContext.setAttribute(Constants.BPM_SERVICE_HOST, getProperty("bpm.service.host"));
		}
		...
		if (p != null) {
			...
			while (e.hasMoreElements()) {
				...
				if (isLogProperty) {
					log.error(key + "---" + getProperty(key));
				}
			}
		}
		log.info("PropertiesHolder holded properties:" + getProperties());
	}
	...
	private void initApplicationProperties(ApplicationContext context) {
		...
		if (StringUtils.isNotEmpty(serverBaseDir) && StringUtils.isNotEmpty(serverBaseDir.trim())) {
			serverBaseDir = System.getProperty(Constants.SERVER_JETTY_BASE);
		}
		// 开始监控
		...
		try {
			...
			for (String string : listFileNames) {
				...
				if (in != null) {
					log.info("jar-properties====" + string);
			...
			List<Resource> normalLocations = new ArrayList<Resource>();

			normalLocations.addAll(CollectionUtils.arrayToList(classpathResources));
			normalLocations.add(new UrlResource("file:" + serverBaseDir + "/application_test.properties"));
			normalLocations.add(new UrlResource("file:" + serverBaseDir + "/application_demo.properties"));

			// 获取加密的Properties
			Resource[] fieldResources = new Resource[] { new UrlResource("file:" + serverBaseDir
					+ "/application_field.properties") };

			Properties[] fieldProperties = new EncryptedPropertiesFactoryBean(fieldResources).getObject();
			...
			if (log.isInfoEnabled()) {
				...
				for (Resource r : fieldResources) {
					propertiesInfo.append(r.toString() + "\n");
				}
				log.info(propertiesInfo.toString());
			}

			// 将已经加载的Properties合并
			List<Properties> loadedProperties = new ArrayList<Properties>();
			loadedProperties.add(systemProperties);
			loadedProperties.addAll(CollectionUtils.arrayToList(fieldProperties));

			// 合并Properties
			Properties result = PropertiesUtils.loadProperties(
					normalLocations.toArray(new Resource[normalLocations.size()]),
					loadedProperties.toArray(new Properties[loadedProperties.size()]));
			...
			if (log.isInfoEnabled()) {
				log.info(" scan application properties consume time(millisecond) : " + (end - start));
			}
		}
	}
