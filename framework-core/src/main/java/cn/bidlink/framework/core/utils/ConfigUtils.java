...
public class ConfigUtils {
    ...
    public static void reload() {
		String path = ConfigUtils.class.getClassLoader().getResource(".").getFile();
    	Properties proot = initClassPath(path);
    	path = ConfigUtils.class.getResource("/").getFile();
	...
	private static Properties initClassPath(String basePath) {
		...
		try {
			...
			for (String string : listFileNames) {
				InputStream in = ConfigUtils.class.getClassLoader().getResourceAsStream(string);
				if (in != null) {
...
//		Properties pchild = new Properties();
		
		Set<Map.Entry<Object, Object>> sets = p.entrySet();
		 for (Map.Entry<Object, Object> entry : sets) {
			System.out.println(entry.getKey()+"="+entry.getValue());
		}
  
//		String xx = MonitorConfigUtils.get("mongo.database");
