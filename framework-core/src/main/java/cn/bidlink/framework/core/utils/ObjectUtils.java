...
public abstract class ObjectUtils extends org.springframework.util.ObjectUtils {
	...
	public static boolean compare(Object first, Object second) {
		...
		//字符串情况
		if((first instanceof String && second==null ) || (second instanceof String && first==null)) {
	...
	public static boolean compare(Object first, Object second, List<String> properties) {
		try {
			for (String property : properties) {
				Method firstMethod = first.getClass().getMethod(
						"get"+ property.substring(0, 1).toUpperCase() + property.substring(1));
				Method secondMethod = second.getClass().getMethod(
						"get"+ property.substring(0, 1).toUpperCase() + property.substring(1));
				...
				if(firstPropertyVal instanceof Map || secondPropertyVal instanceof Map) {
					isEqualed = compare(new HashMap((Map<?, ?>)firstPropertyVal),new HashMap((Map<?, ?>)secondPropertyVal));
				} else if(firstPropertyVal instanceof List || secondPropertyVal instanceof List) {
					isEqualed = compare(new ArrayList((List<?>)firstPropertyVal),new ArrayList((List<?>)secondPropertyVal));
				} else if(firstPropertyVal instanceof Set || secondPropertyVal instanceof Set) {
					isEqualed = compare(new HashSet((Set<?>)firstPropertyVal),new HashSet((Set<?>)secondPropertyVal));
				} else {
